// For use in NodeJS
const BankAccount     = require("./classesnotasmodules/bankaccountES6") ;
const CheckingAccount = require('./classesnotasmodules/checkingES6.js' ) ;
const SavingsAccount  = require('./classesnotasmodules/savingsES6.js' ) ;
// 
   let bunchaAccts = [] ;
   ...
   for ( let objIDX = 0; objIDX < bunchaAccts.length; objIDX++ )
   ...
   let numSavings = bunchaAccts.filter( function( elem ) { return elem.acctID.startsWith('S'); } ).length ;  
   ...
   let totalBalance = bunchaAccts.reduce( function ( total, acctElem ) { return total += acctElem.balance; }, 0 ) ;   
   ...
   let newSavAccts = bunchaAccts.filter( function( elem ) { return elem.acctID.startsWith('S') && elem.balance > 5000 ; } )
                                .map( function( elem ) {  
   ...
   let numEachType = bunchaAccts.reduce( function( accountTypeCounts, anAccount ) { 
